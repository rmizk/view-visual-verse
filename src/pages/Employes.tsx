...
const Employes: React.FC = () => {
  ...
  return (
    <Layout pageTitle="Entreprise" breadcrumbItems={breadcrumbItems}>
      ...
      {/* Time Records Search and Filters - only show in historique tab */}
      {viewMode === 'details' && employeeDetailsTab === 'historique' && (
        <div className="flex justify-between items-center gap-4 w-full">
          <div className="flex items-center gap-2">
            <div className="relative">
              ...
              <Input
                ...
                value={timeRecordsSearch}
                onChange={(e) => setTimeRecordsSearch(e.target.value)}
                className="pl-10 w-64"
              />
            </div>
          </div>
          <div className="flex items-center gap-3">
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="outline" className="w-32">
                  {timeRecordsDateFrom 
                    ? format(timeRecordsDateFrom, 'dd/MM/yyyy', { locale: fr })
                    : 'Date début'
                  }
                </Button>
              </PopoverTrigger>
              <PopoverContent className="w-auto p-0" align="start">
                <CalendarComponent
                  mode="single"
                  selected={timeRecordsDateFrom}
                  onSelect={setTimeRecordsDateFrom}
                  locale={fr}
                />
              </PopoverContent>
            </Popover>
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="outline" className="w-32">
                  {timeRecordsDateTo 
                    ? format(timeRecordsDateTo, 'dd/MM/yyyy', { locale: fr })
                    : 'Date fin'
                  }
                </Button>
              </PopoverTrigger>
              <PopoverContent className="w-auto p-0" align="start">
                <CalendarComponent
                  mode="single"
                  selected={timeRecordsDateTo}
                  onSelect={setTimeRecordsDateTo}
                  locale={fr}
                />
              </PopoverContent>
            </Popover>
            <Select value={timeRecordsStatus} onValueChange={setTimeRecordsStatus}>
              ...
              <tbody className="bg-white divide-y divide-slate-200">
                {timeRecords.map((record) => (
                  <tr key={record.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-slate-900">{record.pause}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-slate-900">{record.heuresTravaillees}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getTimeRecordStatusColor(record.statut)}`}>
                        {getTimeRecordStatusLabel(record.statut)}
                      </span>
                    </td>
                  </tr>
                ))}
